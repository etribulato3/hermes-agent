...
# Adapted from NousResearch/hermes-agent e7433910e96c097ddf34352ea28653e83d951fbb.
# Original store by David Dudok de Wit; upstream interface retained.
...
class RunIdempotencyStore:
        ...
        with self._lock:
            ...
            try:
                ...
                if row is not None:
                    ...
                    outcome = (
                        "reused"
                        if hmac.compare_digest(row["fingerprint"], fingerprint)
                        else "conflict"
                    )
                    return outcome, {
                        "run_id": row["run_id"],
                        ...
                        "owner_started": int(row["owner_started"] or 0),
    ...
    def lookup(
        ...
        *,
        ...
        if row is None:
            return "missing", None
        outcome = "reused" if hmac.compare_digest(row["fingerprint"], fingerprint) else "conflict"
        return outcome, {
            "run_id": row["run_id"],
            "status": json.loads(row["status_json"]),
            "owner_pid": int(row["owner_pid"] or 0),
    ...
    def _prune_stale_terminal_locked(self, now: float) -> None:
        ...
        for row in stale:
            stale_scope = row["scope"]
            stale_key = row["idempotency_key"]
            stale_status = row["status_json"]
            ...
            # Expiry is not proof that the caller durably consumed the effect.
            if terminal and acknowledged_at is not None and expired:
                self._conn.execute(
