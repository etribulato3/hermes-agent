...
// Closed-tab stack (in-memory, session-scoped) for ⌘⇧T reopen — the tile's
// placement is remembered so it comes back where it was.
const closedTileStack: SessionTile[] = []
...
export function closeSessionTile(storedSessionId: string) {
  ...
  if (tile) {
    closedTileStack.push({ anchor: tile.anchor, before: tile.before, dir: tile.dir, storedSessionId })
  }
...
export function reopenLastClosedTile(): void {
  for (let tile = closedTileStack.pop(); tile; tile = closedTileStack.pop()) {
